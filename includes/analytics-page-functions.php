...
function tpae_get_product_view_options() {
	...
	$product_ids = $wpdb->get_results(
        ...
        INNER JOIN {$wpdb->prefix}trendpilot_automation_engine_page_views v ON p.ID = v.product_id
...
function tpae_get_cat_view_options() {
	...
	$categories = $wpdb->get_results(
        ...
        INNER JOIN {$wpdb->prefix}trendpilot_automation_engine_page_views v ON t.term_id = v.category_id
...
function tpae_calculate_product_views() {
	...
	global $wpdb;
	$days_to_look_back = get_option( 'tpae_analytics_period', 7 );
	$date_threshold = wp_date( 'Y-m-d', strtotime( "-{$days_to_look_back} days" ) );
	...
	// Get value '$selected_product' is validated, so $sql is safe
	$view_data = $wpdb->get_results( $wpdb->prepare( $sql, ...$params ) );  // db call ok
...
function tpae_calculate_category_views() {
	...
	global $wpdb;
	$days_to_look_back = (int) get_option( 'tpae_analytics_period', 7 );
	$date_threshold = wp_date( 'Y-m-d', strtotime( "-{$days_to_look_back} days" ) );
...
function tpae_get_recommended_data() {
	global $wpdb;
	...
	// Calculate the date from 'tpae_analytics_period' days ago
	$date_threshold = wp_date( 'Y-m-d', strtotime( "-{$days_to_look_back} days" ) );
	...
	$click_data = $wpdb->get_results( $wpdb->prepare(
		"SELECT product_id, clicked_date, SUM(clicks) as clicks 
		 FROM {$wpdb->prefix}trendpilot_automation_engine_recommended_loop_clicks 
	...
	if ( count( $product_ids ) > 0 ) {
		...
		$product_ids = array_filter( $product_ids, 'is_int' );
...
function tpae_get_recommended_products() {
	global $wpdb;

	// Fetch the number of days to look back from the 'tpae_analytics_period' option
	$days_to_look_back = (int) get_option( 'tpae_analytics_period', 7 );

	// Calculate the date from 'tpae_analytics_period' days ago
	$date_threshold = wp_date( 'Y-m-d', strtotime( "-{$days_to_look_back} days +1 day" ) );

	$recommended_products = $wpdb->get_results( $wpdb->prepare(
		"SELECT product_id, SUM(clicks) AS total_clicks FROM {$wpdb->prefix}trendpilot_automation_engine_recommended_loop_clicks
        WHERE clicked_date >= %s
        GROUP BY product_id
...
function aetp_get_upsell_data() {
	...
	if ( ! $aetp_upsell_product_id ) {
		return; // Exit if no upsell product ID is set
	}

	// Fetch the product details using WooCommerce function
	$product = wc_get_product( $aetp_upsell_product_id );

	if ( ! $product ) {
		return; // Exit if product not found
	}

	// Fetch the last change date for upsell clicks, default to today if not set
	$last_change_date = get_option( 'aetp_upsell_date_last_change', gmdate( 'Y-m-d' ) );

	// Fetching the number of upsell clicks since the last date change
	$upsell_clicks = $wpdb->get_var( $wpdb->prepare(
		"SELECT SUM(upsell_clicks) FROM {$prefix}trendpilot_automation_engine_click_data WHERE clicked_date >= %s AND upsell_clicks IS NOT NULL",
		$last_change_date
	) );
...
function aetp_get_upsell_clicks() {
	global $wpdb;
	...
	$click_data = $wpdb->get_results( $wpdb->prepare(
		"SELECT clicked_date, upsell_clicks FROM {$wpdb->prefix}trendpilot_automation_engine_click_data WHERE clicked_date > %s ORDER BY clicked_date ASC",
		$date_threshold
	) );
